...
pub_date: ""
tags: ["molecular-generation", "generative-models", "drug-discovery", "flow-matching", "deep-learning"]
ai_relevance: 5
relevance: null
innovation: null
methods: null
...
## 精华总结
VFMol框架将个性化变分自编码器潜空间建模与离散流匹配高效采样机制相结合，基于KAN和无分类器引导实现了轻量级的性质引导条件生成。
...
## 摘要（中文）
分子图生成是药物发现中的关键任务。现有方法存在限制：VAE解码器在排列不变性和单次生成上存在问题，DFM模型缺乏对特定分子结构的适应性。本文提出VFMol框架，将个性化VAE潜空间建模与离散空间中的DFM高效采样机制相结合。编码器学习针对每个输入图的后验分布作为生成起点，增强结构保真度和多样性。引入基于KAN和无分类器引导的轻量级性质引导框架，实现无需辅助性质预测器的条件生成。实验证明该方法在分子结构质量和性质可控性上达到最先进水平。
