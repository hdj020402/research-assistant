...
title_zh: "MDIntrinsicDimension: 基于分子动力学轨迹的大分子集体运动的基于维度的分析"
...
pub_date: ""
tags: ["molecular-dynamics", "dimensionality-reduction", "structural-analysis", "computational-chemistry"]
ai_relevance: 3
relevance: null
innovation: null
methods: null
...
## 精华总结
MDIntrinsicDimension是一个开源Python包,通过结合旋转和平移不变的分子投影与最先进的估计器,直接从分子动力学轨迹中估计内在维数,展示了该方法在识别局部柔性、区分结构段和鉴定亚稳态构象方面的优势。

**关键词**: 分子动力学、内在维数、构象空间、生物大分子、降维分析
...
## 摘要（中文）
分子动力学 (MD) 模拟通过生成时间分辨的高维轨迹，提供了对生物分子的结构，动力学和功能的原子见解。分析这些数据受益于估计描述所探索的构象流形所需的最小数量的变量，称为固有维度 (ID)。我们提出了MDIntrinsicDimension，一个开源的Python包，通过将旋转和平移不变的分子投影与最先进的估计器相结合，直接从MD轨迹估计ID。该软件包提供了三种互补的分析模式: 全分子ID，沿序列的滑动窗口和每个二级结构元素。它计算整体ID (单个汇总值) 和瞬时的时间分辨ID，可以显示随时间的过渡和异质性。我们说明了来自DESRES数据集的快速折叠-展开轨迹的方法，证明ID通过突出空间局部灵活性，区分结构片段和识别亚稳态配置来补充常规几何描述符。
