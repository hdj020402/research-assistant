...
title_zh: "BindFlow：使用自由能微扰或MM(PB/GB)SA进行绝对结合自由能计算的免费用户友好型管道"
...
pub_date: ""
tags: ["binding-free-energy", "molecular-dynamics", "computational-chemistry", "software-tool", "force-field"]
ai_relevance: 2
relevance: null
innovation: null
methods: null
...
**BindFlow：使用自由能微扰或MM(PB/GB)SA进行绝对结合自由能计算的免费用户友好型管道**

## 精华总结
本研究将BindFlow确立为绝对结合自由能(ABFE)计算的经过验证的、易于获取的工具，在某些系统和力场中MM(PB/GB)SA计算效果接近FEP，同时计算成本大幅降低。

**关键词**: 结合自由能计算、自由能微扰、MM(PB/GB)SA、分子动力学、BindFlow
...
## 摘要（中文）
我们介绍了BindFlow，一个基于Python的软件，用于在自由能微扰(FEP)或分子力学泊松-玻尔兹曼/广义Born表面积[MM(PB/GB)SA]理论水平上进行自动化的绝对结合自由能(ABFE)计算。BindFlow是免费的、开源的、用户友好的、易于定制的，可在工作站或分布式计算平台上运行，并提供广泛的文档和教程。BindFlow使用GROMACS作为分子动力学引擎，并为小分子力场GAFF、OpenFF和Espaloma提供内置支持。我们通过计算139个配体/靶标对的亲和力来测试BindFlow，涉及八个不同的靶标，包括六个可溶性蛋白质、一个膜蛋白和一个非蛋白质主客体系统。通过Pearson、Kendall和Spearman相关系数的量化，我们发现BindFlow预测与实验的一致性总体上与该领域的黄金标准相似。有趣的是，我们发现MM(PB/GB)SA对于某些系统和力场，所获得的相关性接近FEP的结果，同时仅需要较少部分的计算成本。本研究将BindFlow确立为一个经过验证的、易于使用的ABFE计算工具。
