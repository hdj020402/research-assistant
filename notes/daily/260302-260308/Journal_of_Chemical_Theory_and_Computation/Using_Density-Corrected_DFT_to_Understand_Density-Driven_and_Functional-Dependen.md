...
title_zh: "利用密度校正密度泛函理论理解水合电子从头算模拟中的密度驱动误差与泛函依赖性误差"
...
pub_date: ""
tags: ["dft-methods", "hydrated-electron", "quantum-chemistry", "error-analysis", "molecular-dynamics"]
ai_relevance: 2
relevance: null
innovation: null
methods: null
...
**利用密度校正密度泛函理论理解水合电子从头算模拟中的密度驱动误差与泛函依赖性误差**

## 精华总结
该研究使用密度校正DFT (DC-DFT)方法来理解和改进DFT在水合电子体系模拟中的密度驱动误差，通过用Hartree-Fock密度替换自洽密度来减轻电荷非定域化问题。研究表明虽然DC-DFT可以减少密度驱动误差，但由于破坏了PBEh泛函中的误差相消机制，实际上使预测结果与实验值的偏离更大，揭示DFT对水合电子的模拟困难主要源于DFT的固有近似而非密度驱动误差。

**关键词**: 密度校正DFT、水合电子、密度驱动误差、电荷非定域化、分子动力学模拟
