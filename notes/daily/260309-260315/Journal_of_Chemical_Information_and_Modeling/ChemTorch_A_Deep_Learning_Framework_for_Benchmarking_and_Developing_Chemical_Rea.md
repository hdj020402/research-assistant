...
title_zh: "ChemTorch：用于化学反应性质预测模型基准测试和开发的深度学习框架"
...
pub_date: ""
tags: ["deep-learning", "chemical-reaction-prediction", "benchmarking-framework", "graph-neural-networks", "reproducibility"]
ai_relevance: 5
relevance: null
innovation: null
methods: null
...
**ChemTorch：用于化学反应性质预测模型基准测试和开发的深度学习框架**

## 精华总结
本工作提出ChemTorch开源框架，通过模块化管道、标准化配置和内外分布数据分割器，简化化学反应建模中的模型开发、实验、超参数调优和基准测试。

**关键词**: 深度学习、化学反应预测、基准测试框架、图神经网络、分布外评估
...
## 摘要（中文）
化学反应建模对于理解动力学机制和预测反应系统的可能结果至关重要。量子力学计算准确但成本高昂。深度学习提供了更快的替代方案，但软件生态碎片化阻碍了进展。本工作提出ChemTorch框架，通过模块化管道、标准化配置和分布内外评估数据分割器，简化了模型开发、实验、超参数调优和基准测试。框架比较了四种代表性模态用于势垒高度预测，结果突出了结构感知模型的优势和分布外条件下的性能下降。
