...
title_zh: "烟气中的co2捕获: 用于吸附剂发现的高保真力场和机器学习框架"
...
pub_date: ""
tags: ["machine-learning", "co2-capture", "force-field-development", "adsorbent-discovery", "molecular-simulation"]
ai_relevance: 4
relevance: null
innovation: null
methods: null
...
**烟气中的co2捕获: 用于吸附剂发现的高保真力场和机器学习框架**

## 精华总结
开发了基于Exp-PE势的高保真范德华力场和CO2吸附数据库,通过四极矩响应描述符改进机器学习模型的预测精度,解决了传统Lennard-Jones力场在描述CO2相互作用中的系统偏差问题。该框架成功筛选出COF-50和COF-364等高性能吸附剂,其工作容量超过现有多孔材料。

**关键词**: CO2捕获、机器学习、力场开发、吸附剂发现、多孔材料
...
## 摘要（中文）
来自烟道气的CO2对于减轻化石燃料衍生的排放至关重要，其中吸附剂性能直接决定了工艺能量效率和工艺成本。尽管机器学习 (ML) 已成为加速吸附剂发现的强大工具，但其预测准确性从根本上受到基础训练数据的物理可靠性的限制，这是 “垃圾输入，垃圾输出” (GIGO) 问题的体现。大多数现有的CO2吸附数据库依赖于lennard-jones (LJ) 力场，其在描述CO2-CO2和CO2-framework相互作用方面的缺陷，特别是在高压下，将系统偏差引入ML模型。为了解决这个问题，我们基于exp-pe电势开发了物理上精确的范德华力场，并构建了高保真的CO2吸附数据库。在此数据集的基础上，我们引入了四极响应描述符，该描述符明确捕获了CO2的各向异性静电，从而提高了ML预测的准确性。该框架确定了高性能COF/MOF吸附剂，包括COF-50 (ΔNCO2 = 13.58 mol/kg) 和COF-364 (ΔNCO2 = 12.43 mol/kg)，其工作容量超过了目前报道的多孔材料。
