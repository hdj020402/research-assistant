...
pub_date: ""
tags: ["deep-learning", "chemical-reaction-modeling", "benchmark-framework", "molecular-representation", "hyperparameter-tuning"]
ai_relevance: 5
relevance: null
innovation: null
methods: null
...
## 精华总结
本文介绍了ChemTorch，一个开源框架，通过模块化管道、标准化配置和内置数据分割器来简化化学反应建模的模型开发、实验、超参数调优和基准测试，特别强调了分布内和分布外评估的重要性。

**关键词**: 深度学习、化学反应预测、基准测试框架、分子表示、机器学习
...
## 摘要（中文）
化学反应的建模对于理解动力学机制和预测反应系统的可能结果至关重要。量子力学计算是准确的，但往往过于昂贵。深度学习已经成为一种更快的选择，但由于分散的软件生态系统阻碍了重用、公平比较和可重复性，因此进展缓慢。我们介绍了ChemTorch，这是一个开源框架，它通过模块化管道、标准化配置和内置数据分离器来简化模型开发、实验、超参数调整和基准测试，用于分发内外评估。我们将ChemTorch设想为社区驱动的方法开发和化学反应建模中可重复的基准测试的基础。作为迈向统一基准的第一步，我们比较了RDB7数据集上用于障碍物高度预测的四种代表性模式，包括基于指纹，序列，图形和3d的方法。我们的结果突出了结构知情模型的明显优势，以及在分布条件下性能急剧下降，突出了严格基准测试的重要性。
