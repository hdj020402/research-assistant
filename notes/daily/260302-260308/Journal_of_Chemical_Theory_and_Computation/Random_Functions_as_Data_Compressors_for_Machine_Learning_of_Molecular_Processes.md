...
pub_date: ""
tags: ["machine-learning", "dimensionality-reduction", "molecular-dynamics", "protein-folding", "feature-compression"]
ai_relevance: 4
relevance: null
innovation: null
methods: null
...
**随机函数作为分子过程机器学习的数据压缩器**

## 精华总结
研究表明随机非线性投影可以压缩大特征空间，加快计算速度而不会显著丧失信息。该方法在蛋白质折叠轨迹分析中保留了原始高维特征空间的关键静态和动态信息。

**关键词**: 特征压缩、随机投影、分子动力学、蛋白质折叠、机器学习
...
## 摘要（中文）
机器学习 (ML) 正在迅速改变分子动力学模拟的执行和分析方式，从材料建模到蛋白质折叠和功能的研究。ML算法通常用于学习构象景观的低维表示，并将轨迹聚类为相关的亚稳态。这些算法中的大多数都需要选择描述感兴趣问题的少量特征。虽然深度神经网络可以处理大量的输入特征，但是训练成本随着输入大小的增加而增加，这使得对于大多数实际感兴趣的问题来说，选择特征子集是强制性的。在这里，我们表明随机非线性投影可用于压缩大特征空间，并使计算更快，而不会大量损失信息。我们描述了一种产生随机投影的有效方法，然后举例说明了蛋白质折叠的一般程序。对于我们的测试一下案例NTL9和villin头件的双正亮氨酸变体，我们发现随机压缩保留了原始高维特征空间的核心静态和动态信息，使轨迹分析更加鲁棒。

## 笔记
<!-- 在此处添加阅读笔记 -->

## Links
- [Full Article](https://app.acspubs.org/e/er?s=1913652004&lid=109586&pci=CACSR000013623915&elqTrackId=017814fc29fa461e9a6bf65d23801867&elq=51f4713da496490ea94be1416e307abc&elqaid=32172&elqat=1&elqak=8AF55C4D98404DCE0933CD5D9D557C4D8EC8DF669D1886211F10624EBC003E655BFD)
