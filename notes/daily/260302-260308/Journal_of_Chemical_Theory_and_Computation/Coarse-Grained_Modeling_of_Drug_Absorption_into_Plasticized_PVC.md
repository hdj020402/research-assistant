...
## 摘要（中文）
输注期间的药物损失可由水性制剂和基于聚合物的医疗装置 (最通常由增塑聚氯乙烯 (PVC) 制成) 之间的界面处的复杂吸附和吸收现象引起。尽管分子模拟方法在描述吸附现象方面取得了成功，但它们固有地受到其长度和时间尺度的限制，这妨碍了它们在研究吸收过程中的应用。在这里，我们使用Martini 3粗粒框架扩展了这种分子理解，以探索包含DEHT或TOTM增塑剂的增塑PVC基质中的吸收。我们使用自上而下的方法来完善分子间的相互作用。在优化溶质-水和溶质-PVC非键合相互作用以重现实验分配数据后，我们结合了平均力 (PMF)，自由能扰动 (BAR) 和长平衡模拟的潜力，以绘制热力学和动力学方面的吸附。这项工作建立了一个可转移的粗粒度框架，用于在较长的时间范围内对药物-聚合物相互作用进行建模，将原子见解与实验观察联系起来，并为包括赋形剂和复杂聚合物制剂在内的全输注系统模拟铺平道路。
