...
pub_date: ""
tags: ["feature-compression", "molecular-dynamics", "protein-folding", "dimensionality-reduction", "random-projections"]
ai_relevance: 4
relevance: null
innovation: null
methods: null
...
**随机函数作为机器学习分子过程的数据压缩器**

## 精华总结
研究表明随机非线性投影可以压缩大特征空间,在不造成实质性信息损失的情况下加快计算速度。
...
## 摘要（中文）
机器学习(ML)正在迅速改变分子动力学模拟的执行和分析方式,从材料建模到蛋白质折叠和功能研究。ML算法通常被用来学习构象景观的低维表示,并将轨迹聚类为相关的亚稳态。这些算法中的大多数都需要选择少量描述感兴趣问题的特征。虽然深度神经网络可以处理大量输入特征,但训练成本随着输入大小而增加,这使得对于大多数实际问题,选择特征子集是必要的。在本研究中,我们展示了随机非线性投影可以用于压缩大特征空间,加快计算速度而不会产生实质性的信息损失。我们描述了一种产生随机投影的高效方法,然后以蛋白质折叠为例说明了一般程序。对于我们的测试案例NTL9和双去亮氨酸变体的villin头部域,我们发现随机压缩保留了原始高维特征空间的核心静态和动态信息,使轨迹分析更加稳健。
